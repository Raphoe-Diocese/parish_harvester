...
- [ ] **doing** · 2026-08-23 · Recipe success — 13 Problems-tab parishes. **Clonleigh / Ballymoney / Glenariffe / Kincasslagh live ok**. **Castleblayney** wait for harvest `parish_status`. **Clones doing** (not done): homepage lists `/uploads/downloads/Sunday 23rd August 2026.pdf` with a raw space; urllib `InvalidURL`. Encode spaces in scrape/fetch — do not pin a dated filename; do not mark ok until a harvest on main writes `parish_status`. **Ballinascreen doing** (not done): this-week HTML page is real (proved 23/08/2026) `https://www.parishofballinascreen.com/ballinascreen-desertmartin-parishes-23_august_2026` HTTP 200 `text/html` title `BALLINASCREEN & DESERTMARTIN PARISHES 23_AUGUST_2026` with this-week notices; no PDF and no large scan (image_stack found 0). Recipe switched to wix_dated_slug + print_to_pdf. Do not mark done until a harvest on main writes `parish_status` ok. Content gap: Bangor 14/06, Dunsford 05/07, Holy Cross (230826.pdf still 12/07 body), Malin April (August URL 404), Hannahstown 07/06, Saint Malachy’s Feb 2025, St Oliver 28/06, St Patrick’s 05/07, Stranorlar 28/06. Skip Carrick / Lisburn / Tyholland. · `parishes/recipes/`, `harvester/replay.py`, `harvester/utils.py` `quote_http_url`
- [ ] **doing** · 2026-08-23 · Clones this-week file is **real** (re-proved 23/08/2026): `https://www.clonesparish.com/uploads/downloads/Sunday%2023rd%20August%202026.pdf` HTTP 200, `application/pdf`, 417727 bytes, `%PDF-1.7`. Raw space URL raises `InvalidURL`. Encode `%20` in HTTP-scrape. Do not pin a dated filename. **Leave open until a harvest writes `parish_status` for clones.** · `parishes/recipes/clogher/clones.json`, `harvester/utils.py` `quote_http_url`, `harvester/replay.py`
