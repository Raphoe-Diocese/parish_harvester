...
- [ ] **doing** · 2026-08-21 · Back room **View bulletin** on each Problems row opens the PDF harvest saved (`Bulletins/current/` or `Bulletins/stale/`). Too-old files were previously thrown away, so today’s stale rows have no file until the next harvest. · `extension/sidepanel.js`, `harvester/report.py`
