...
- [ ] **doing** · 2026-08-23 · Back to Top arrow (Divi-style ↑) must show when the reader scrolls the INNER PDF or OCR box, not only when the whole page scrolls. Live Raphoe already had capture-on-document JS, but the script ran before `#scroll-top-btn` and never bound the 850px boxes themselves. Fix: wait for the button / create it; bind `.pdf-inpage-pages` + `#ocr-panel` directly; re-bind after PDF.js rebuilds pages (`inner-2`, `?v=20260823u`). **Leave open until the arrow is watched on live inner scroll** https://www.parishpress.ie/dioceses/raphoe/ · `harvester/site_chrome.py` `scroll_top_js`, `docs/assets/pdf-inpage-viewer.js`
