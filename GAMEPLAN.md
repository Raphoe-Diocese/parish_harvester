...
- **Fix the 7 `no_evidence` parishes (this turn)** — look up live sites. Galloon + Lisnaskea-Maguiresbridge are **aliases** of already-ok Clogher rows (`newtownbutler`, `lisnaskeamaguiresbridge`) — skip, do not harvest twice. Carryduff this week is Word `/app/uploads/…6th-September-2026.docx`. Ardkeen this week is `23-Sunday-OT-A.png`. Dundrum (MCN 465) and Loughguile (MCN 317) have CloudFront newsletters. Cappagh moved to `cappaghparish.com/bulletins` (`/b/13` = 30/08 — stale until they post). Problems filter defaults to Raphoe, so these never showed until **All dioceses**.
...
**This turn:** packed **1.61.23** `.crx` on Pages. `updates.xml` points the packed id at the CRX. One drag-install, then Update can fetch. Not proved until his card is packed and later shows a newer version after Update.
...
**Not a miss (site has no 06/09 file):** Malin (newest April); Ardara; Inver; Saint Malachy’s `bulletin.pdf` still Feb 2025.

**Tawnawilly 06/09 evening:** listing “Sunday 6th Sept” href is `Sunday-Sept-06-26.pdf` (HTTP 200, printed Sunday 6th Sept 2026 / 23rd Sunday). `Sunday-6th-Sept.pdf` 404s. Predicted harvest must try Month-DD-YY. Not in the mega until Full harvest.
