...
https://www.antrimparish.com
    ...
    def test_deploy_pages_builds_extension_update_assets(self) -> None:
        ...
        self.assertIn('github.event.workflow_run.name == \'Harvest Parish Bulletins\'', workflow)
        ...
        self.assertIn("Download mega PDFs from latest harvest when missing", workflow)
        self.assertIn("fallback — trigger had none", workflow)
        ...
        self.assertIn("Apply workflow docs snapshots to checkout", workflow)
        self.assertIn("harvest-docs-*", workflow)
        self.assertIn("ocr-docs-*", workflow)
        self.assertIn('pdfs=(mega_pdf/*_mega_bulletin.pdf)', workflow)
        self.assertIn('if [ "${EVENT_NAME}" = "workflow_run" ] && [ "${TRIGGERING_WORKFLOW}" = "Harvest Parish Bulletins" ] && [ ${#pdfs[@]} -eq 0 ]; then', workflow)
        self.assertIn("deploying diocese pages without fresh mega PDFs", workflow)
    ...
    def test_ocr_bulletin_workflow_configuration(self) -> None:
        ...
        self.assertIn("actions/download-artifact@v4", workflow)
        self.assertIn('pattern: "*-mega-bulletin-pdf"', workflow)
        self.assertIn("steps.harvest_run.outputs.run_id", workflow)
        ...
        self.assertIn("from harvester.ocr_mega_resolve import decide_ocr_mega_source", workflow)
        self.assertIn("Skip OCR when harvest has no mega PDF", workflow)
        self.assertIn("steps.harvest_run.outputs.skip != '1'", workflow)
        self.assertIn("has_full_harvest_docs_artifact", workflow)
        self.assertIn("pick_run_with_mega_artifact", workflow)
        self.assertIn("Staged from checkout", workflow)

    def test_harvest_workflow_requires_mega_pdf_for_full_runs(self) -> None:
        workflow = (REPO_ROOT / ".github" / "workflows" / "harvest.yml").read_text(encoding="utf-8")
        self.assertIn("HARVEST_MEGA_PDF: \"1\"", workflow)
        self.assertIn("Require mega PDFs for full harvest", workflow)
        self.assertIn("Upload mega PDF artifacts", workflow)
        self.assertIn("all-mega-bulletin-pdf", workflow)
        self.assertIn("if-no-files-found: error", workflow)
        self.assertIn("github.event.inputs.target_parish == ''", workflow)
        require_idx = workflow.index("Require mega PDFs for full harvest")
        upload_idx = workflow.index("Upload mega PDF artifacts")
        self.assertLess(require_idx, upload_idx)
    ...
    def test_ocr_convert_provider_fallback_order(self) -> None:
        ...
        mistral_idx = source.find("Mistral OCR (mistral-ocr-latest) on PDF ...")
        gemini_idx = source.find("Running image OCR with Gemini (gemini-2.5-flash) fallback ...")
        github_idx = source.find("Running image OCR with GitHub Models (gpt-4o-mini) fallback ...")
        ...
        self.assertLess(gemini_idx, github_idx)
        self.assertLess(github_idx, openai_idx)
