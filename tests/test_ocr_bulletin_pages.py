...
class OcrBulletinPageTests(unittest.TestCase):
    ...
    def test_extract_ocr_fragment_and_render_viewer_page(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("pdf-wait-note", html_output)
            self.assertIn("can take a few moments to open", html_output)
            self.assertNotIn('class="mobile-jump-download"', html_output)
            self.assertNotIn("#panel-pdf .az-row { display: none !important; }", html_output)
            ...
            # One Open PDF / Download pair on the box; OCR keeps its text tab.
            ...
            self.assertIn("/assets/pdf-inpage-viewer.js?v=20260901h", html_output)
    ...
    def test_mobile_pdf_inpage_viewer_helpers(self) -> None:
        ...
        self.assertIn("pdf-open-newtab", panel)
        self.assertIn('target="_blank"', panel)
        self.assertIn("noopener noreferrer", panel)
        ...
        self.assertIn("/assets/pdf-inpage-viewer.js?v=20260901h", boot)
    ...
    def test_ocr_search_sits_above_the_letter_and_zoom_rows(self) -> None:
        ...
        jump_pdf = html_output.split("function jumpPdf", 1)[1].split("function jumpOcr", 1)[0]
        self.assertIn("parishPressScrollPdfToPage(page)", jump_pdf)
        self.assertIn("slots after page 1 do not exist yet", jump_pdf)
        self.assertLess(
            jump_pdf.index("parishPressScrollPdfToPage(page)"),
            jump_pdf.index("[data-page="),
            "Jump-to must start before it looks for a painted slot",
        )
        ...
        # Phone and desktop both hide sideways overflow so the page fits the box.
        self.assertGreaterEqual(
            html_output.count(".pdf-inpage-pages { overflow-x: hidden !important; }"), 2
        )
        self.assertNotIn(".pdf-inpage-pages { overflow-x: auto !important; }", html_output)
        self.assertNotIn(".pdf-inpage-page-slot { max-width: none; }", html_output)
    ...
    def test_pdf_inpage_viewer_asset_streams_first_page(self) -> None:
        ...
        self.assertEqual(PDF_INPAGE_VIEWER_VERSION, "20260901h")
        ...
        self.assertIn("PDFDataRangeTransport", text)
        ...
        self.assertIn("pdf-open-newtab", text)
        self.assertIn("Do not preventDefault", text)
        ...
        self.assertIn("tries++ < 80", scroll_fn)
        ...
        self.assertNotIn("fillPhoneNativeLink", text)
        self.assertNotIn("pdf-phone-native", text)
        self.assertNotIn("is-phone-native", text)
        self.assertNotIn("This week's PDF", text)
        self.assertNotIn("PDF.js is desktop-only", text)
        self.assertNotIn("armPhonePdfNewTab", text)
        ...
        run_fn = text[run_idx:run_end]
        self.assertGreater(run_idx, 0)
        self.assertNotIn("loadPdfJs()", run_fn)
        self.assertIn("startViewer(host, pdfUrl)", run_fn)
        self.assertNotIn("fillPhoneNativeLink", run_fn)
        self.assertNotIn("isMobileView()", run_fn)
        load_idx = text.find("function loadPdfJs()")
        load_fn = text[load_idx:text.find("function resolvePdfUrl")]
        self.assertNotIn("isMobileView()", load_fn)
        start_head = text[text.find("function startViewer"):text.find("function activateWrap")]
        self.assertNotIn("if (isMobileView()) return;", start_head)
        pw_start = text.find("function pageWidth")
        pw_end = text.find("function ensureSlot", pw_start)
        self.assertGreater(pw_start, 0)
        self.assertGreater(pw_end, pw_start)
        page_width_fn = text[pw_start:pw_end]
        self.assertIn("clientWidth", page_width_fn)
        self.assertNotIn("720", page_width_fn)
        self.assertNotIn("Math.max(boxWidth, 720)", text)
        self.assertNotIn("overflow-x:auto!important", text.replace(" ", ""))
        self.assertIn("overflow-x:hidden!important", text.replace(" ", ""))
        self.assertIn("orientationchange", text)
        self.assertIn("Math.min(window.devicePixelRatio || 1, 2)", text)
        self.assertNotIn("Math.min(dpr, 1)", text)
        ...
        self.assertIn("pdf-inpage-viewer.js?v=20260901h", loader.read_text(encoding="utf-8"))
    ...
    def test_live_gortahork_ocr_has_mega_body_and_visible_850(self) -> None:
        ...
        self.assertIn("/assets/pdf-inpage-viewer.js?v=20260901h", diocese)
    ...
    def test_live_diocese_html_ships_inpage_viewer(self) -> None:
        ...
        for rel in (
            ...
            self.assertIn("slots after page 1 do not exist yet", html_live, rel)
            self.assertIn("Download PDF", html_live, rel)
            self.assertNotIn('class="mobile-jump-download"', html_live, rel)
            self.assertNotIn("#panel-pdf .az-row { display: none !important; }", html_live, rel)
            self.assertNotIn("Distraction-free view", html_live, rel)
            self.assertIn("pdf-wait-note", html_live, rel)
            self.assertIn("can take a few moments to open", html_live, rel)
