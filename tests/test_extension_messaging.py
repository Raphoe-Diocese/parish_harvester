...
class ExtensionMessagingTests(unittest.TestCase):
    ...
    def test_problems_back_room_uses_parish_status_cards(self) -> None:
        ...
        self.assertIn("problems-more", js)
        self.assertIn("waiting for test", js)
        self.assertIn("_problemsCategoryLabel", js)
        self.assertIn("harvestNote", js)
        self.assertIn("_problemsOpenHarvestedPdf", js)
        self.assertIn("_problemsHarvestPdfCandidates", js)
        self.assertIn("_problemsPdfViewerTabUrl", js)
        self.assertIn("_problemsOpenPdfViewerTab", js)
        self.assertIn('chrome.runtime.getURL("pdf_tab.html")', js)
        self.assertNotIn("chrome.tabs.create({ url: download, active: true })", js)
        bg = (REPO_ROOT / "extension" / "background.js").read_text(encoding="utf-8")
        self.assertIn("ph_guard_pdf_download", bg)
        self.assertIn("chrome.downloads.cancel", bg)
        self.assertTrue((REPO_ROOT / "extension" / "pdf_tab.html").is_file())
        self.assertTrue((REPO_ROOT / "extension" / "pdf_tab.js").is_file())
        self.assertIn("pdf.worker.min.js", (REPO_ROOT / "extension" / "pdf_tab.js").read_text(encoding="utf-8"))
    ...
    def test_trainer_matches_github_parish_status(self) -> None:
        ...
        self.assertEqual(manifest.get("version"), "1.61.38")
        ...
        self.assertIn('type: "dismiss_toolbar"', content)
        self.assertIn("ph_toolbar_hidden_hosts", content)
        self.assertNotIn("_locateRecipeOnGithub", (REPO_ROOT / "extension" / "background.js").read_text(encoding="utf-8"))
        self.assertNotIn("_problemsFilterActionableRows", sidepanel)
